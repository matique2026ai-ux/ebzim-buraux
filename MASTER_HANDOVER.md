...
- **ثورة الخريطة التفاعلية (Heritage Discovery Engine):** الخريطة الآن تسحب البيانات حياً من Wikipedia API وتعرض المشاريع الميدانية بدقة على خريطة قمر صناعي عالية الدقة.
- **تثبيت الصور العالمية:** استبدلنا روابط Wikimedia غير المستقرة بـ Unsplash API لضمان عمل الصور عالمياً دون مشاكل CORS.
- **تحكم الإغلاق (UX):** أضفنا زر (X) شفاف واحترافي على كروت تفاصيل الخريطة لتسهيل التصفح.
- **توطين البيانات:** فصلنا المشاريع عن الأخبار هيكلياً ووظيفياً، مع دعم كامل للغات الثلاث.

### 🧭 أين وصلنا تقنياً؟

- **الباكاند (NestJS):** مستقر جداً، يدعم الـ CRUD الكامل لكل الوحدات، ومربوط بـ Render auto-deploy.
- **الفرونت (Flutter):** يستخدم أحدث تقنيات `flutter_animate` و `Riverpod`. جميع التحذيرات (Lints) في ملف الـ Handover تم تصفيرها.
...
1. **نظام الطبقات في الخريطة:** تطوير زر تبديل (Toggle) للسماح للمستخدم بالاختيار بين رؤية "المشاريع المؤسساتية" أو "الاكتشافات العالمية" بشكل منفصل.
2. **تحسين الأداء (Caching):** تنفيذ `Provider` كاش للمشاريع التي تم تحميلها من Wikipedia لتقليل استهلاك الـ API وسرعة التصفح.
3. **توطين محتوى Wikipedia:** حالياً الوصف يظهر بلغة البحث، نحتاج لآلية ذكية لجلب الوصف باللغة المختارة (AR/FR/EN) إن توفرت.
4. **نظام الإشعارات:** البدء في ربط الـ Backend بـ Firebase أو نظام إشعارات داخلي للتنبيه عن المشاريع الجديدة.
