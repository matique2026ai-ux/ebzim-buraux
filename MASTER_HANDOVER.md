...
- **الفرونت (Flutter):** يستخدم أحدث تقنيات `flutter_animate` و `Riverpod`. تم إضافة نظام تصفية الطبقات (Map Layers Logic).
...
1. **تحسين الأداء (Caching):** تنفيذ `Provider` كاش للمشاريع التي تم تحميلها من Wikipedia لتقليل استهلاك الـ API وسرعة التصفح.
2. **توطين محتوى Wikipedia:** حالياً الوصف يظهر بلغة البحث، نحتاج لآلية ذكية لجلب الوصف باللغة المختارة (AR/FR/EN) إن توفرت.
3. **نظام الإشعارات:** البدء في ربط الـ Backend بـ Firebase أو نظام إشعارات داخلي للتنبيه عن المشاريع الجديدة.
